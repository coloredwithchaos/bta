...
if ($seopressor_is_active && $seopressor_has_permission) {
	...
	$enable_role_settings = WPPostsRateKeys_CustomRoles::enable_role_settings();
	
	global $wpdb;
	
	// prepare query to get total record
	$query_total = "select count(id) from " . $wpdb->prefix . WPPostsRateKeys::$db_prefix . WPPostsRateKeys_CustomRoles::$table_roles_capabilities . " where role_type = 'wp'";
	...
	$query = "select * from " . $wpdb->prefix . WPPostsRateKeys::$db_prefix . WPPostsRateKeys_CustomRoles::$table_roles_capabilities . " where role_type='wp' $order_by_query limit $offset, $limit";
	...
	foreach( $result as $key=>$row )
	{
		...
		{
			$arr = explode( ',', $row->capabilities );
		}
		else
		{
			$arr = array();
		}
		$result[$key]->capabilities = $arr;
	}
	
	
	$sort = get_admin_url() . "admin.php?page=seopressor-role-settings&paged=" . $paged . "&order_by=1&order=" . ( $order_by == 1 ? $order_target : 'asc' );
