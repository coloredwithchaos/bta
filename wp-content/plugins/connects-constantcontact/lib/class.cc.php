...
class cp_cc {
	 ...
	 * @access 	public
	 */
	function set_action_type($action_type='contact')
	{
		$this->action_type = (strtolower($action_type)=='contact') ? 'ACTION_BY_CONTACT' : 'ACTION_BY_CUSTOMER';
	}
	...
	function get_all_lists($action = 'lists', $exclude = 3, $callback = '')
	{
		...
		if(count($lists) > 0):
			...
			$callback = ($callback) ? $callback : array("cp_cc", "sort_lists");
			if(is_array($lists)):
...
';
  ...
  <content type="application/vnd.ctct+xml">
	 ...
	 * @access 	public
	 */
	function create_contact($email, $name = '', $lists = array(), $additional_fields = array())
	{
