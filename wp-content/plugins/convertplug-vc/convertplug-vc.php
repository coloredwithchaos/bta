<?php
/*
Plugin Name: Convert Plus Addons for WPBakery Page Builder.
Plugin URI: https://www.convertplug.com/plus
Author: Brainstorm Force
Author URI: https://www.brainstormforce.com
Version: 2.0.2
Description: Easy to use form builder for WPBakery Page Builder users. Now you can create form with unlimited fields, save form submission data in campaigns, see analytics, graphs & best of all - third party sync with Mailchimp, MyMail etc.
...
if(!defined('CONVERTPLUG_VC_VERSION')){
	define('CONVERTPLUG_VC_VERSION', '2.0.2');
}
...
if (! class_exists( "ConvertPlug_VC" )) {
	class ConvertPlug_VC {
		...
		// Function to Notice for Install & Active WPBakery Page Builder.
		function admin_notice_for_cp_activation() {
			$is_multisite = is_multisite();
			$is_network_admin = is_network_admin();
			if( ( $is_multisite && $is_network_admin ) || !$is_multisite )
				echo '<div class="updated"><p>'.__('The','convertplug_vc').' <strong>'.CP_PLUS_NAME.' Addon for WPBakery Page Builder</strong> '.__('plugin requires','convertplug_vc').' <strong>'.CP_PLUS_NAME.'</strong> '.__('Plugin installed and activated.','convertplug_vc').'</p></div>';
		}
		...
		// Function to Notice about WPBakery Page Builder Version
		function admin_notice_for_version() {
			$is_multisite = is_multisite();
			$is_network_admin = is_network_admin();
			if(($is_multisite && $is_network_admin) || !$is_multisite)
				echo '<div class="updated"><p>'.__('The','convertplug_vc').' <strong>Convert Plus Addon for WPBakery Page Builder</strong> '.__('plugin requires','convertplug_vc').' <strong>WPBakery Page Builder</strong> '.__('version 3.7.2 or greater.','convertplug_vc').'</p></div>';
		}
		...
		function admin_notice_for_vc_activation() {
			...
			$is_network_admin = is_network_admin();
			if(($is_multisite && $is_network_admin) || !$is_multisite)
				echo '<div class="updated"><p>'.__('The','convertplug_vc').' <strong>'.CP_PLUS_NAME.' Addon for WPBakery Page Builder</strong> '.__('plugin requires','convertplug_vc').' <strong>WPBakery Page Builder</strong> '.__('Plugin installed and activated.','convertplug_vc').'</p></div>';
		}
			...
			if(function_exists('vc_map')) {
				vc_map(
					array(
						...
						"controls" => "full",
						"category" => "Convert Plus WPBakery Addon",
		...
		function cpvc_custom_form_element () {
            ...
            if( isset($smile_lists) && count( $smile_lists ) > 0 ) {
			...
			if(function_exists('vc_map'))
			{
				...
				vc_map(
					array(
					   ...
					   "category" => __("Convert Plus","convertplug_vc"),
